...
void Editor::addVertexObject(int i, int j, Platform * platform)
{
...
inline VertexObject * Editor::getVertexObject(int i)
{
...
void Editor::Init()
{
	...
	Editor::LoadGameWorldFromXml("Resources/GameData/game_world_data.xml", m_pWorld, m_pObjectPool, this);
}
...
void Editor::ExecuteCommand(char command)
{
	...
	if (command == 0) {
		if (!save_flag) {
			WriteWorldStructureToFile("Resources/GameData/game_world_data.xml", m_pWorld, m_pObjectPool, this);
			save_flag = true;
		}
	}
	...
	if (command == 3) m_putter.Trigger(ObjectId::OID_ANIMAL);
...
void Editor::WriteWorldStructureToFile(const char * filename, World*pWorld, ObjectPool*pObjectPool, Editor*pEditor)
{
	tinyxml2::XMLDocument doc;
	...
	for (int i = 0; i < pWorld->GetEntities()->size(); i++) {
		...
		for (int j = 0, n = platform.m_polygons.size(); j < n; j++) {
			...
			for (int k = 0, m = platform.m_polygons[j].size(); k < m; k++) {
				...
				VertexObject*vo = (pEditor!=NULL?pEditor->getVertexObject(count++):NULL);
		...
		while (pPlatform) {
			...
			while (pPolygon) {
				...
				while (pVertex) {
					...
					if(pEditor) pEditor->addVertexObject(count_i,count_j, newPlatform);
					...
					while (pAABBObject) {
						...
						if (pEditor) pEditor->m_picker.attach(attachedObject,pEditor->m_verexObjects.back());
